...
class TestHome(TestCase):
    ...
    def test_gets_right_mobilizer_url(self, resp_mock):
        ...
        req = self.rf.get('/')
        req.locale = settings.MOBILIZER_LOCALE_LINK.keys()[-1]
        self.view(req)
        ctx = resp_mock.call_args[0][2]
        self.assertEqual(ctx['mobilizer_link'],
                         settings.MOBILIZER_LOCALE_LINK[req.locale])
    ...
    def test_gets_default_mobilizer_url(self, resp_mock):
        ...
        ctx = resp_mock.call_args[0][2]
        self.assertEqual(ctx['mobilizer_link'],
                         settings.MOBILIZER_LOCALE_LINK['en-US'])
